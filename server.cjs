...
const app = express()
const resend = new Resend('re_CcmJ8A2Q_76YM1RjQZycWVcuyXyku7R9s')
const TO_EMAIL = 'shadabsayyed1932@gmail.com'
const PORT = 3001

app.use(cors({ origin: ['http://localhost:5200', 'http://localhost:5173', 'http://localhost:5174'] }))
app.use(express.json())
app.use(express.urlencoded({ extended: true }))

app.get('/api/health', (_req, res) => res.json({ ok: true }))

function buildEmail(name, email, phone, service, message) {
  ...
  return [
    ...
    '<table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0">',
    '<tr>',
    '<td width="47%" bgcolor="#141414" style="background-color:#141414;padding:20px;vertical-align:top;">',
    '<p style="margin:0 0 6px 0;font-family:Arial,Helvetica,sans-serif;font-size:9px;letter-spacing:3px;text-transform:uppercase;color:#3D4E5A;">Name</p>',
    '<p style="margin:0;font-family:Arial,Helvetica,sans-serif;font-size:16px;font-weight:900;text-transform:uppercase;color:#D7E2EA;">' + name + '</p>',
    '</td>',
    '<td width="6%" style="padding:0 8px;">&nbsp;</td>',
    '<td width="47%" bgcolor="#141414" style="background-color:#141414;padding:20px;vertical-align:top;">',
    '<p style="margin:0 0 6px 0;font-family:Arial,Helvetica,sans-serif;font-size:9px;letter-spacing:3px;text-transform:uppercase;color:#3D4E5A;">Email</p>',
    '<p style="margin:0;font-family:Arial,Helvetica,sans-serif;font-size:13px;font-weight:bold;color:#B600A8;word-break:break-all;">' + email + '</p>',
    '</td>',
    '</tr>',
    '<tr><td colspan="3" style="height:8px;">&nbsp;</td></tr>',
    '<tr>',
    '<td width="47%" bgcolor="#141414" style="background-color:#141414;padding:20px;vertical-align:top;">',
    '<p style="margin:0 0 6px 0;font-family:Arial,Helvetica,sans-serif;font-size:9px;letter-spacing:3px;text-transform:uppercase;color:#3D4E5A;">Phone</p>',
    '<p style="margin:0;font-family:Arial,Helvetica,sans-serif;font-size:16px;font-weight:900;color:#D7E2EA;">' + (phone || '&mdash;') + '</p>',
    '</td>',
    '<td width="6%" style="padding:0 8px;">&nbsp;</td>',
    '<td width="47%" bgcolor="#141414" style="background-color:#141414;padding:20px;vertical-align:top;">',
    '<p style="margin:0 0 6px 0;font-family:Arial,Helvetica,sans-serif;font-size:9px;letter-spacing:3px;text-transform:uppercase;color:#3D4E5A;">Service</p>',
    '<p style="margin:0;font-family:Arial,Helvetica,sans-serif;font-size:13px;font-weight:900;text-transform:uppercase;color:#D7E2EA;">' + (service || '&mdash;') + '</p>',
    '</td>',
    '</tr>',
    '</table></td></tr>',
...
app.post('/api/contact', async (req, res) => {
  ...
  try {
    ...
    console.log('Sending email, HTML length:', html.length)
...
function startServer() {
  const server = app.listen(PORT, () => {
    ...
    console.log('Delivering to ' + TO_EMAIL)
  })
  server.on('error', (err) => {
